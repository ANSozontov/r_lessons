...
udm <- st_read("udmmap/udm.shp")
adm <- st_read("udmmap/adm.shp") %>% 
    select(name) %>% 
    st_cast("LINESTRING") %>% 
    st_crop(xmin = 51, xmax = 54, 
            ymin = 55.8, ymax = 58.6)
    
lakes1 <- st_read("udmmap/lakes1.shp")
lakes2 <- st_read("udmmap/lakes2.shp")
big.rivers <- st_read("udmmap/bg.rivers.shp") 
sm.rivers <- st_read("udmmap/sm.rivers.shp") 
tx <- readxl::read_xlsx("data_example_22.03.2022.xlsx", sheet = "taxa2")

base <- ggplot() + 
    theme_dark()+
    geom_sf(data = udm, fill = "white", color = "black", linewidth = 0.5) +
    geom_sf(data = adm, color = "black", linewidth = 0.5) + 
    geom_sf(data = big.rivers, color = "blue", linewidth = 0.75) +
    geom_sf(data = sm.rivers,  color = "blue", linewidth = 0.25) +
    geom_sf(data = lakes1, color = "blue", fill = "blue") + 
    geom_sf(data = lakes2, color = "blue", fill = "blue") +
    scale_x_continuous(limits = c(51, 54.5), expand = c(0,0), breaks = 51:54) +
    scale_y_continuous(limits = c(55.8, 58.6), expand = c(0,0))


set.seed(1); my.dots <- tx$sp %>% 
    `names<-`(., .) %>% 
    as.list() %>% 
    # .[1:2] %>% 
    map(~sample(c(15:35, seq(40, 100, by = 5)), 1)) %>% 
    map(~tibble(
        lon = runif(.x, 51, 54.5),
        lat = runif(.x, 55.8, 58.6)) %>% 
            st_as_sf(coords = c("lon", "lat"), crs = 4326, remove = FALSE)
    ) %>% 
    map(~filter(.x, !sapply(st_intersects(.x$geometry, udm$geometry), is_empty)) %>% 
            mutate(id = 1:nrow(.), type = sample(1:sample(1:10, 1), nrow(.), replace = TRUE))
    )
    

for(i in 1:length(my.dots)){ 
    p <- base + 
        geom_sf(data = my.dots[[i]], 
                aes(fill = factor(type)), 
                shape = 21, 
                color = "black", 
                size = 4) + 
        geom_text(data = my.dots[[i]], aes(lon, lat, label = id), size = 3) + 
        labs(x = "", y = "", title = paste0("map for ", names(my.dots)[i])) + 
        theme(legend.position = "none",
              plot.title = element_text(size = 13))
    print(i)
    ggsave(filename = paste0("export/map for ", i, ".png"), p, 
           width = 3, height = 5)
}
